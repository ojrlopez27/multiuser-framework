...
- [**EX16_MasterCallsSlaves:**] (Examples/SaraProject/src/main/java/edu/cmu/inmind/multiuser/sara/examples/MasterCallsSlaves.java) This example is pretty similar to EX15_Wholepipeline, however, unlike Ex15_WholePipeline, in this example the master MUF (Sara MUF) calls its slaves MUF's (i.e., Dialogue MUF, etc.). In order to run this example, you have to define the connection information of your slaves MUF's into the file services.json (i.e., replace 'services[_remove_this].json' by 'services.json').
