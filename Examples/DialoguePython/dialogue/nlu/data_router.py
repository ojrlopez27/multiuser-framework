...
from nlu.model import Metadata, InvalidModelError, Interpreter
from nlu.train import do_train
    ...
    def create_interpreter(self, meta, config):
        ...
        for component_name in meta.pipeline:
            try:
                ...
                nlu.components.init_component(component, context, model_config)
...
class DataRouter(object):
    ...
    def _create_query_logger(response_log_dir):
        ...
        # yulun shuts off the logging
        response_log_dir = None
    ...
    def __create_emulator(self):
        ...
        from nlu.emulators import NoEmulator
        return NoEmulator()
    ...
    def start_train_process(self, data):
        ...
        #_config["data"] = f.name
        _config["data"] = './data/data.json'
        train_config = NLUConfig(cmdline_args=_config)
