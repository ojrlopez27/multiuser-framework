...
def load_data_from_json(filename):
    ...
    intent = data['nlu_data'].get("intent_examples", list())
    entity = data['nlu_data'].get("entity_examples", list())
...
def guess_format(files):
    # type: ([str]) -> str
    """Given a set of files, tries to guess which data format is used. encoding="utf-8-sig")"""
...
def load_data(resource_name, language, luis_data_tokenizer=None, fformat=None):
    ...
    if fformat == LUIS_FILE_FORMAT:
        from nlu.tokenizers import tokenizer_from_name
        ...
        return load_data_from_json(files[0])
    else:
