...
class Broker(threading.Thread):
    ...
    INTERNAL_SERVICE_PREFIX = bytes("mmi.", "UTF-8")
    ...
    def mediate(self):
        ...
        while True:
            ...
            if items:
                ...
                if (Constants.C_CLIENT == header):
                    self.process_client(sender, msg)
                elif (Constants.S_WORKER == header):
    ...
    def process_worker(self, sender, msg):
        ...
        elif (Constants.S_REPLY == command):
            if (worker_ready):
                # Remove & save client return envelope and insert the
                # protocol header and service name, then rewrap envelope.
                client = msg.pop(0)
                empty = msg.pop(0) # ?
                msg = [client, b'', Constants.C_CLIENT, worker.service.name] + msg
                self.socket.send_multipart(msg)
                self.worker_waiting(worker)
            else:
                self.delete_worker(worker, True)

        elif (Constants.S_HEARTBEAT == command):
            if (worker_ready):
                worker.expiry = time.time() + 1e-3*self.HEARTBEAT_EXPIRY
            else:
                self.delete_worker(worker, True)

        elif (Constants.S_DISCONNECT == command):
    ...
    def delete_worker(self, worker, disconnect):
        ...
        if disconnect:
            self.send_to_worker(worker, Constants.S_DISCONNECT, None, None)
    ...
    def service_internal(self, service, msg):
        ...
        msg = msg[:2] + [Constants.C_CLIENT, service] + msg[2:]
        self.socket.send_multipart(msg)
    ...
    def send_heartbeats(self):
        ...
        if (time.time() > self.heartbeat_at):
            for worker in self.waiting:
                self.send_to_worker(worker, Constants.S_HEARTBEAT, None, None)
    ...
    def dispatch(self, service, msg):
        ...
        while service.waiting and service.requests:
            ...
            self.send_to_worker(worker, Constants.S_REQUEST, None, msg)
    ...
    def send_to_worker(self, worker, command, option, msg=None):
        ...
        if option is not None:
            msg = [option] + msg
        msg = [worker.address, b'', Constants.S_WORKER, command] + msg
