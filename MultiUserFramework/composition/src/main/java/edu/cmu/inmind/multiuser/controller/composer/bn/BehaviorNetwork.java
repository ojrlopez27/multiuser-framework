...
public class BehaviorNetwork {
    ...
    private double delta = 50; // 90 the amount of activation energy taken away by the protected goals per protected goal.
    ...
    private transient boolean shouldWaitForSync = false;
    ...
    public boolean isExecutable() {
        return !shouldWaitForSync && executable;
    }
...
//                System.out.println("activation-level " + behaviors.get(i).getName() + ": " + behaviors.get(i).getActivation());
    ...
    public void setPremiseWeight(String behaviorName, String premiseName, Double weight) {
        for(Behavior behavior : behaviors){
            if(behavior.getName().equals(behaviorName)){
                for(List<Premise> premises : behavior.getPreconditions()){
                    for(Premise premise : premises){
                        if(premise.getLabel().equals(premiseName)){
                            premise.setWeight( weight );
                            return;
                        }
                    }
                }
    ...
    public boolean isWaitingForUserSelection() {
        return shouldWaitForUserSelection;
    }

    public boolean isBehaviorPicked() {
        return pickBehaviorNow;
    }

    public boolean isWaitingForSync(){
        return shouldWaitForSync;
    }
